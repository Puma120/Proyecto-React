...
const loginUser = async (email, password) => {
    try {
        ...
        if (userDoc.exists()) {
            ...
            return { user: userCredential.user, error: null, userData: userData };
        } else {
            console.log("No such document!");
            return { user: userCredential.user, error: "No such document", userData: null };
        }
    } catch (error) {
        console.log("Error:", error.message);
        return { user: null, error: error.message, userData: null };
    }
}
